...
::: {.proof}
We assume that $d$ is represented by a formula $\delta(x,y)$ and the set $\ulcorner T \urcorner$ is represented by a formula $\varphi_T(v_0)$ in $T$, and we choose $\theta = \neg \varphi_T$, so that from the Diagonal Lemma we obtain the existence of a formula $G$ such that
$$
