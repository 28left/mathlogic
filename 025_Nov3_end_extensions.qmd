...
# End Extensions of the Standard Model{#end-extensions}


::: {#def-end-ext-ord}
\
Let $(A,<^A)$ and $(B,<)$ be linearly ordered sets. We say $(A,<)$ is an **initial segment** of $(B,<^B)$ (and $(B,<)$ is an **end extension** of $(A,<)$) if $A \subseteq B$ and
$$
\text{for all} \; x \in A,y \in B: (y<^B x \Rightarrow y \in A),
$$
i.e., $B$ does not add elements below any element of $A$.
:::

We use the notation
$$
(A, <) \subseteq_{\operatorname{end}} (B,<)
$$
to indicate that $(A,<)$ is an initial segment of $(B,<)$.

Every model of $\mathsf{PA}^-$ is a linearly ordered set. So we can speak of one model being an initial segment (or end extension) of another, meaning their respective linear orders have this property.
