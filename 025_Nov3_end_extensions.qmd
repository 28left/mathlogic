...
and
$$
\mathsf{PA}^- \vdash x \le  \underline{k} \to  (x = \underline{0} \; \vee \ldots \vee \; x = \underline{k})
$$
...
::: {.proof}
We prove (i) by induction on the formula structure of $\varphi(\vec{v})$. Only the case of a bounded quantifier needs to be addressed. Since $\mathcal{M}$ is an end extension of $\mathcal{N}$, $M$ does not insert any new elements below an element of $N$, so that a bounded quantifier means the same thing in both structures.
:::
